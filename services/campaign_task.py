...
def create_campaign_task(campaign_id: int, in_task: CampaignTaskCreate, current_user: UserModel, db: Session):
    campaign_db = (
...
def update_campaign_task(task_id: int, in_task: CampaignTaskUpdate, current_user: UserModel, db: Session):
    task = db.query(CampaignTaskModel).filter(CampaignTaskModel.id == task_id).first()
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
