...
from schemas.campaign_task import CampaignTaskCreate, TaskPriority, TaskStatus, CampaignTaskUpdate
...
def get_campaign_task_by_id(
    ...
    task = db.query(CampaignTaskModel).filter(CampaignTaskModel.id == id).first()
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def update_campaign_task(id: int, in_task: CampaignTaskUpdate, current_user: UserModel, db: Session):
    task = db.query(CampaignTaskModel).filter(CampaignTaskModel.id == id).first()
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    update_data = in_task.model_dump(exclude_unset=True)
    is_owner = campaign.owner_id == current_user.id
    is_creator = task.created_by_id == current_user.id
    is_assignee = task.assignee_id == current_user.id
    if not is_owner and not is_creator and not is_assignee:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def delete_campaign_task(id: int, current_user: UserModel, db: Session):
    task = db.query(CampaignTaskModel).filter(CampaignTaskModel.id == id).first()
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
