...
def search_campaigns(keyword: str | None, current_user: UserModel, db: Session):
    query = (
        db.query(CampaignModel)
        .join(
            CampaignMemberModel,
...
def find_campaign_by_id(campaign_id: int, current_user: UserModel, db: Session):
    campaign_db = (
        db.query(CampaignModel)
        .join(
            CampaignMemberModel,
...
def update_campaign(campaign_id: int, in_campaign: CampaignUpdate, current_user: UserModel, db: Session):
    campaign_db = (
        db.query(CampaignModel)
        .filter(
            CampaignModel.id == campaign_id,
...
def delete_campaign(campaign_id: int, current_user: UserModel, db: Session):
    campaign_db = (
        db.query(CampaignModel)
        .filter(
            CampaignModel.id == campaign_id,
