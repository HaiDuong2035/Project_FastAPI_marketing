...
def create_task_comment(id: int, in_comment: CampaignTaskCommentCreate, current_user: UserModel, db: Session):
...
def get_task_comments(id: int, current_user: UserModel, db: Session):
    task = db.query(CampaignTaskModel).filter(CampaignTaskModel.id == id).first()
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    return (
        ...
        .filter(CampaignTaskCommentModel.task_id == id)
