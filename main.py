...
from routers.campaign_task import router as campaign_task_router
...
def validation_exception_handler(req: Request, exc: RequestValidationError):
    response = create_response(status.HTTP_422_UNPROCESSABLE_CONTENT, "Lỗi chuẩn hóa nội dung", req, error = str(exc.errors))
    return JSONResponse(
